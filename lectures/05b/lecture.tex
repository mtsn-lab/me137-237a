...
%		% Commands to include a figure:
		\begin{itemize}
			\item Explain the physical limitations of air cooling in high-TDP (Thermal Design Power) environments.
			\item Identify the key components used to transfer heat away from a chip (TIM, Spreaders, Heat Pipes).
	...
	\begin{frame}{The Heat Challenge: Why Liquid?}
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item Limited heat transfer coefficient ($\alpha$).
			\end{itemize}
	...
	\begin{frame}{Thermal Interface Materials (TIM)}
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item \textbf{Bond Line Thickness (BLT):} Minimizing thickness reduces total thermal resistance ($R_{th}$).
			\end{itemize}
		\end{itemize}
		\begin{block}{Thermal Resistance Equation}
			$R_{TIM} = \frac{t}{k \cdot A}$
		\end{block}
	\end{frame}

	\begin{frame}{Spreading the Heat: Spreaders \& Heat Pipes}
		\begin{itemize}
			\item \textbf{Heat Spreaders:}
			\begin{itemize}
				\item High-conductivity metal (usually Copper) to distribute heat laterally.
				\item Reduces "spreading resistance."
			\end{itemize}
			\item \textbf{Heat Pipes \& Vapor Chambers:}
			\begin{itemize}
				\item Use \textbf{phase change} (evaporation/condensation) to transport heat.
				\item \textbf{Heat Pipe:} 1D transport via capillary action.
				\item \textbf{Vapor Chamber:} 2D spreading via phase change; extremely efficient for high-flux chips.
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Case Study: Aquasar Technology}
		\begin{itemize}
			\item \textbf{Concept:} A hot water cooled supercomputer prototype designed for direct energy reuse.
...
\smallframetitle
\begin{frame}
	\frametitle{\href{https://datahub.berkeley.edu/user-redirect/git-pull?repo=https://github.com/mtsn-lab/me137-237a&subPath=examples/blasiusPohlhausenEquations.ipynb}{Microelectronics cooling: Solution to Blasius}}

% This derivation comes from Ref.\cite{bejan2004convection}.
		\begin{figure}
			\includegraphics[width=0.4\textwidth]{../figures/blasius.pdf}
%			\vspace{-0.5cm}
		 \caption{Blasius profile. Solved numerically in 1908. Read his biography \href{https://doi.org/10.1007/s00348-002-0582-9}{here}.}
		\end{figure}
...
\smallframetitle
\begin{frame}
	\frametitle{\href{https://datahub.berkeley.edu/user-redirect/git-pull?repo=https://github.com/mtsn-lab/me137-237a&subPath=examples/blasiusPohlhausenEquations.ipynb}{Microelectronics cooling: Solution to Pohlhausen}}

% This derivation comes from Ref.\cite{bejan2004convection}.
		\begin{figure}
			\includegraphics[width=0.4\textwidth]{../figures/pohlhausen.pdf}
%			\vspace{-0.5cm}
